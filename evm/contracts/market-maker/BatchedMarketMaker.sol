...
contract BatchedBancorMarketMaker is AccessControl, ReentrancyGuard {
  ...
  string private constant ERROR_INSUFFICIENT_POOL_BALANCE =
    "MM_INSUFFICIENT_POOL_BALANCE";
  string private constant ERROR_TRANSFER_FROM_FAILED =
    "MM_TRANSFER_FROM_FAILED";
  ...
  mapping(uint256 => BatchWithFee) public batches;
  ...
  function openBuyOrder(
    ...
    require(
      !_batchIsCancelled(_currentBatchId(), _collateral),
      ERROR_BATCH_CANCELLED
    );
    require(
      _collateralValueIsValid(_buyer, _collateral, _value),
  ...
  function openSellOrder(
    ...
    require(
      curveSupply - _amount > minCurveSupply,
      ERROR_INSUFFICIENT_POOL_BALANCE
    );
  ...
  function claimBuyOrder(
    ...
    require(
      _userIsBuyer(_batchId, _collateral, _buyer),
  ...
  function claimSellOrder(
    ...
    require(!_batchIsCancelled(_batchId, _collateral), ERROR_BATCH_CANCELLED);
    require(
      _userIsSeller(_batchId, _collateral, _seller),
      ERROR_NOTHING_TO_CLAIM
    );
  ...
  function claimCancelledBuyOrder(
    ...
    require(
      _batchIsCancelled(_batchId, _collateral),
      ERROR_BATCH_NOT_CANCELLED
    );
    require(
      _userIsBuyer(_batchId, _collateral, _buyer),
      ERROR_NOTHING_TO_CLAIM
  ...
  function getCollateralToken(
    address _collateral
  ) external view returns (bool, uint256, uint256, uint32, uint256) {
  ...
  {
    Batch storage batch = batches[_batchId].batches[_collateral];
  ...
  function getCollateralPricePPM(
    address _collateral
  ) external view returns (uint256) {
    return
      getStaticPricePPM(
        curveSupply + tokensToBeMinted,
        ERC20(_collateral).balanceOf(address(this)) -
          collateralsToBeClaimed[_collateral],
        collaterals[_collateral].reserveRatio
      );
  }
  ...
  function withdrawNFT(
    ...
    ERC721(_token).safeTransferFrom(
      address(this),
      this.beneficiary(),
      _tokenId
    );
  ...
  function updateCurveSupply(
    uint256 _supply
  ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    curveSupply = _supply;
  }

  function updateMinCurveSupply(
    uint256 _minSupply
  ) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function _staticPricePPM(
    ...
    return
      (uint256(PPM) * uint256(PPM) * _balance) /
      (_supply * uint256(_reserveRatio));
  }
  ...
  function _collateralValueIsValid(
    ...
    return (ERC20(_collateral).balanceOf(_buyer) >= _value &&
      ERC20(_collateral).allowance(_buyer, address(this)) >= _value);
  }

  function _bondAmountIsValid(
    address _seller,
    uint256 _amount
  ) internal view returns (bool) {
  ...
  function _collateralIsWhitelisted(
    address _collateral
  ) internal view returns (bool) {
  ...
  function _batchIsCancelled(
    uint256 _batchId,
    address _collateral
  ) internal view returns (bool) {
    return batches[_batchId].batches[_collateral].cancelled;
  }
  ...
  function _poolBalanceIsSufficient(
    address _collateral
  ) internal view returns (bool) {
    return
      ERC20(_collateral).balanceOf(address(this)) >=
  ...
  function _slippageIsValid(Batch storage _batch) internal view returns (bool) {
    uint256 staticPricePPM = _staticPricePPM(
      _batch.supply,
      _batch.balance,
      _batch.reserveRatio
    );
  ...
  function _buySlippageIsValid(
     ...
     */
    if (
      _batch.totalBuyReturn *
        _startingPricePPM *
        (PCT_BASE + _maximumSlippage) >=
  ...
  function _currentBatch(
    address _collateral
  ) internal returns (uint256, Batch storage) {
    uint256 batchId = _currentBatchId();
    BatchWithFee storage batchWithFee = batches[batchId];
    Batch storage batch = batchWithFee.batches[_collateral];

    if (!batchWithFee.initialized) {
      /**
       * NOTE
      ...
      /**
       * NOTE
       * realSupply(batchWithFee) = totalSupply(batchWithFeeInitialization) + tokensToBeMinted(batchWithFeeInitialization)
       ...
       * as such totalSupply(batchWithFee) + tokenToBeMinted(batchWithFee) will always equal totalSupply(batchWithFeeInitialization) + tokenToBeMinted(batchWithFeeInitialization)
       */
      batchWithFee.realSupply = curveSupply + tokensToBeMinted;
      batchWithFee.buyFeePct = buyFeePct;
      batchWithFee.sellFeePct = sellFeePct;
      batchWithFee.formula = formula;
      batchWithFee.initialized = true;

      emit NewBatch(
        batchId,
        batchWithFee.realSupply,
        batchWithFee.buyFeePct,
        batchWithFee.sellFeePct,
        address(batchWithFee.formula)
      );
    }
    ...
    if (!batch.initialized) {
      /**
       * NOTE
       * supply(batch) = realSupply(batchWithFee) + virtualSupply(batchInitialization)
      ...
      /**
       * NOTE
       * balance(batch) = poolBalance(batchInitialization) - collateralsToBeClaimed(batchInitialization) + virtualBalance(batchWithFeeInitialization)
      ...
      batch.balance =
        ERC20(_collateral).balanceOf(address(this)) +
        collaterals[_collateral].virtualBalance -
        collateralsToBeClaimed[_collateral];
  ...
  function _openBuyOrder(
    ...
    // deduct fee
    uint256 fee = (_value * batches[batchId].buyFeePct) / PCT_BASE;
    ...
    // update batch
    batch.totalBuySpend = batch.totalBuySpend + value;
    batch.buyers[_buyer] = batch.buyers[_buyer] + value;
    ...
    tokensToBeMinted =
      (tokensToBeMinted - deprecatedBuyReturn) +
      batch.totalBuyReturn;
    collateralsToBeClaimed[_collateral] =
      collateralsToBeClaimed[_collateral] -
      deprecatedSellReturn +
      batch.totalSellReturn;
  ...
  function _openSellOrder(
    ...
    // update batch
    batch.totalSellSpend = batch.totalSellSpend + _amount;
    batch.sellers[_seller] = batch.sellers[_seller] + _amount;
    ...
    tokensToBeMinted =
      ...
      batch.totalBuyReturn;
    collateralsToBeClaimed[_collateral] =
      (collateralsToBeClaimed[_collateral] - deprecatedSellReturn) +
      batch.totalSellReturn;

    // sanity checks
    require(_slippageIsValid(batch), ERROR_SLIPPAGE_EXCEEDS_LIMIT);
    require(
      _poolBalanceIsSufficient(_collateral),
      ERROR_INSUFFICIENT_POOL_BALANCE
    );
  ...
  function _claimBuyOrder(
    ...
    Batch storage batch = batches[_batchId].batches[_collateral];
    uint256 buyReturn = (batch.buyers[_buyer] * batch.totalBuyReturn) /
      batch.totalBuySpend;
  ...
  function _claimSellOrder(
    ...
    Batch storage batch = batches[_batchId].batches[_collateral];
    uint256 saleReturn = (batch.sellers[_seller] * batch.totalSellReturn) /
      batch.totalSellSpend;
    uint256 fee = (saleReturn * batches[_batchId].sellFeePct) / PCT_BASE;
  ...
  function _claimCancelledSellOrder(
    ...
    if (amount > 0) {
      tokensToBeMinted = tokensToBeMinted - amount;
      curveSupply += amount;
  ...
  function _updatePricing(
    ...
    uint256 resultOfSell = (batch.totalSellSpend * staticPricePPM) /
      uint256(PPM);
    ...
    if (resultOfSell > batch.totalBuySpend) {
      ...
      batch.totalBuyReturn =
        (batch.totalBuySpend * uint256(PPM)) /
        staticPricePPM;
      ...
      uint256 remainingSellReturn = batches[_batchId]
        .formula
        .calculateSaleReturn(
          batch.supply,
          batch.balance,
      ...
      uint256 remainingBuyReturn = batches[_batchId]
        .formula
