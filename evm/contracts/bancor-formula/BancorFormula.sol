...
contract BancorFormula is IBancorFormula, Utils {
  ...
  constructor() {
    initializeMaxExpArray();
  }

  function initializeMaxExpArray() private {
   ...
   *
   * Formula:
   * PurchaseReturn = ContinuousTokenSupply * ((1 + ReserveTokensReceived / ReserveTokenBalance) ^ (ReserveRatio / 1000000) - 1)
   *
   * @param _startingPrice       price of the first token (no decimals), measured in collateral amount
  ...
  function calculatePurchaseReturn(
    ...
    require(
      virtualReserveBalance > 0,
      "Starting price incorrect: needs to be higher than (1*10^decimals)/reserveRatio"
    );

    // special case if the ratio = 100%
    if (_reserveRatio == MAX_RATIO)
      return (virtualSupply * _depositAmount) / virtualReserveBalance;

    uint256 result;
    uint8 precision;
    uint256 baseN = _depositAmount + virtualReserveBalance;
  ...
  function calculateSaleReturn(
    ...
    //TODO
    revert("Sale is not allowed, formula not supported yet");
  ...
  function calculateCrossReserveReturn(
    ...
    //TODO
    revert("Cross reserve sale is not allowed, formula not supported yet");
