...
export default function TokenPage() {
	...
	const [nfts, setNfts] =
		useState<[NonFungibleResourcesCollectionItemVaultAggregated]>()
	...
	const fetchCurrentFunding = useCallback(
		async (tokenDetails: TokenDetails | null) => {
			...
			try {
				...
				const adequateResourceVault = get(result, 'vaults', []).find(
					(vault) =>
						get(vault, 'resourceAddress') === tokenDetails.factoryComponentId,
				)
				if (
					adequateResourceVault &&
					typeof adequateResourceVault === 'object'
				) {
	...
	const fetchNFTs = useCallback(async (tokenDetails: TokenDetails | null) => {
		...
		try {
			...
			if (matchingNFTs.length > 0) {
				...
				setNfts(
					matchingNFTs as [NonFungibleResourcesCollectionItemVaultAggregated],
				)
	...
	const handleConfirmInvestment = async () => {
		...
		try {
			...
			if (dialogAction === ActionType.Buy) {
				...
				manifest = claimManifest(token.factoryComponentId, {
					accountId,
					nftTokenAddress: token.id,
					amount: investmentAmount,
				})
			}

			console.log('manaifest', manifest)
										...
										>
											...
											<Button
												...
												onClick={() => {
													...
													const transactionManifest = claimManifest(
														token.factoryComponentId,
														{
															accountId,
													...
													console.log('manifets', transactionManifest)
