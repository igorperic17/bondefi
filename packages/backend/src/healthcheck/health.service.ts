...
import configuration, { ConfigSettings } from "@config/configuration";
...
export class HealthService {
  private readonly configuration = new LazyInitializer<object>(() => {
    const result = { ...configuration() } as Omit<ConfigSettings, "secrets"> &
      Partial<Pick<ConfigSettings, "secrets">>;
